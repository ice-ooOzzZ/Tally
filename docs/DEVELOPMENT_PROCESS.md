...
## 流水线档位(2026-07-09 负责人决策)

按模块重要性/成本选择档位,门禁标准(无 CRITICAL/HIGH、AC 全绿、防未来函数)两档一致:

- **全量档**(M0.5、M1 T1.1 已用):② 2 审查(code-reviewer + python-reviewer)+ ③ 2 测试(tdd-guide + 独立 AC/压测)+ ⑥ 2 回归(测试 + 产品经理)。最严谨、最贵。
- **精简档(M1 T1.2 起默认)**:① 1 开发 + ② 1 合并审查(代码质量+Python 一体)+ ③ 1 测试(覆盖+AC 边界一体)+ ④ 改动文档 + ⑤ 合并 + ⑥ 1 回归(测试+产品验收一体)。省 token、更快;关键/高风险模块(如策略正确性、组合资金逻辑、回测撮合)可临时升回全量档。

## 开发 Agent 纪律(2026-07-09 补充,防挂死)

开发/修复 Agent **禁止派生任何子 Agent、禁止等待任何子任务**——只做:改代码 → 自测门禁 → commit → 回报。审查/测试一律由主控统一编排。(教训:曾有开发 Agent 自起子 Agent 又死等而挂死。)
